...
8. Receipt extraction and secure payout onboarding are server-side integrations. Configure `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `MINDEE_API_KEY`, `MINDEE_MODEL_ID`, and `STRIPE_SECRET_KEY` as Cloudflare Worker secrets/vars before enabling them. Mindee V2 keys are sent only from the Worker; the browser never receives either provider secret.
